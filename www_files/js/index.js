...
function loadItem(itemId){
	...
	$.getJSON("/api/groupSchedule/"+itemId, function(calConts){
		$.getJSON("/api/amicaVI", function(foods){
			...
			for(var i = 0;i<calConts.length;i++){
				...
				var leftOut = parseInt(md5(preferedName),16);
				var desiredColor = colorList[leftOut%colorList.length];
			...
			for(var i = 0;i<foods.MenusForDays.length;i++){

				var cfood = foods.MenusForDays[i];
				var startTime = new Date(cfood.Date);
				startTime.setHours(12);
				startTime.setMinutes(1);
				var endTime  = new Date(cfood.Date);
				endTime.setHours(12);
				endTime.setMinutes(44);
				
				events.push({
					title: "RUOKAILU, VISAMÄKI AMICA",
					start: startTime,
					end: endTime,
					color: "#795548",
					data:cfood
